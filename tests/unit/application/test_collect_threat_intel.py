...
class StubSource:
    """Async test double for ThreatSource."""
...
class TestCollectThreatIntelUseCase:
    ...
    @pytest.mark.asyncio
    async def test_whitelist_filters_ips(self):
        # Arrange
        src = StubSource("Src", {
    ...
    async def test_deduplication_across_sources(self):
        # Arrange — both sources report the same IP
        ...
        # Assert — 3 unique IPs, shared one has both sources
    ...
    @pytest.mark.asyncio
    async def test_updates_health_records(self):
        # Arrange
        health_repo = StubHealthRepo()
    ...
    @pytest.mark.asyncio
    async def test_overlap_metrics_computed(self):
        # Arrange
